...
class EarningsScanner:
    def __init__(self, symbols: list, threshold_pct: float = 10.0, state_file: str = "scanner_state.json"):
        """
    ...
    def _send_notification(self, symbol: str, pct_change: float, curr_ni: float, prev_ni: float, filing_date: str):
        """
    ...
    def run(self):
        ...
        for symbol in self.symbols:
            ...
            if len(financials) < 2:
                logger.warning(f"[{symbol}] Not enough historical data to compare quarters. Skipping.")
                continue
            ...
            if not last_seen_date:
                curr_q = curr_report.get("fiscal_period", "")
                curr_y = curr_report.get("fiscal_year", "")
                logger.info(f"[{symbol}] First time scanning. Discovered latest report {curr_y} {curr_q} filed on {curr_filing_date}. Initializing state.")
            ...
            if curr_filing_date and curr_filing_date > last_seen_date:
                curr_q = curr_report.get("fiscal_period", "")
                curr_y = curr_report.get("fiscal_year", "")
                logger.info(f"[{symbol}] *** NEW REPORT DISCOVERED *** {curr_y} {curr_q} (Filed: {curr_filing_date})")
                ...
                if curr_ni is not None and prev_ni is not None:
                    ...
                    if pct_change >= self.threshold_pct:
                        self._send_notification(symbol, pct_change, curr_ni, prev_ni, curr_filing_date)
                else:
                    logger.warning(f"[{symbol}] Could not extract net income from the financials payload.")
