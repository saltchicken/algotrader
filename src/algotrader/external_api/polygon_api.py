...
class PolygonClient:
    ...
    def get_ticker_details(self, symbol: str) -> dict:
        ...
        logger.error(f"Failed to fetch Polygon ticker details for {symbol}: {response.text}")
        return {}
    ...
    def get_historical_financials(self, symbol: str, limit: int = 1) -> list:
        ...
        if response.status_code != 200:
            logger.error(f"Failed to fetch Polygon financials for {symbol}: {response.text}")
            return []
        ...
        for i in range(len(raw_results)):
            ...
            if i + 1 < len(raw_results):
                ...
                if not (curr_q_str.startswith('Q') and next_q_str.startswith('Q') and curr_y and next_y):
                    continue
                ...
                while (expected_older_q != next_q or expected_older_y != int(next_y)) and len(continuous_results) < limit:
                    logger.warning(
